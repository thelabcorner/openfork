...
- EDIT `packages/opencode/src/quota/providers/openrouter.ts` — fix `valueLabel` to `"$${formatMoney(remaining)} left · $${formatMoney(totalUsage)} spent"` (match OpenChamber + `HANDOFF-quota-reimplementation.md` note). Keep `usedPercent:null` (balance metric).
