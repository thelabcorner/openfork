...
An **independent product**. Packages: `ui`, `web`, `electron`, `vscode`, `mobile`, `docs`. It talks to official OpenCode through `@opencode-ai/sdk/v2` and a bundled/installed CLI. Their `AGENTS.md` literally says: do not modify `../opencode`; it is a separate repository.
...
Root `AGENTS.md` today is upstream's file plus two fork paragraphs. After isolation it must say, in this order:
...
Skills own workflows. `AGENTS.md` owns always-on constraints. That is the OpenChamber lesson that *does* transfer.
