...
| D10 | Agent docs | Replace root `AGENTS.md` with the fork guide. Add `.opencode/skills/upstream-sync`. Keep upstream style/API rules. | Agents currently have no sync doctrine. The two merge commits are the only source. |
...
- `AGENTS.md` as routing + always-on rules; skills as workflows.
...
- Root `AGENTS.md` + `FORK.md` + `upstream-sync` skill exist and describe tag-merge + union conflicts.
