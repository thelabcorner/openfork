...
| File | Job |
|---|---|
| `AGENTS.md` | Always-on rules. Short. Routing table to skills. |
...
| Package `AGENTS.md` files | Keep upstream + existing desktop/app notes. |

OpenChamber's lesson: `AGENTS.md` is routing, not a novel. Workflows live in skills.
...
- Root `AGENTS.md` **replaces** the current file, but **keeps** the style guide, testing, typecheck, API Architecture, and V2 Session Core sections verbatim (they are still correct for KEEP packages).
