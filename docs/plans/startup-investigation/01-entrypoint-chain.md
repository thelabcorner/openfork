...
- Canonical clock: lane-5 harness `raw/harness/run-trial.ps1` (monotonic `Stopwatch` line stamps, `ELECTRON_ENABLE_LOGGING=1`, `NO_COLOR=1`, OS-level window detection via `MainWindowTitle` on new electron PIDs, tree-kill teardown, JSONL per trial). Reused verbatim per ground rule #2.
