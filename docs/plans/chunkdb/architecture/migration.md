...
**Supersedes for this lane:** `../ideation/migration.md` where they conflict. The size-gate and the epoch-flip mechanics are **re-derived**: the in-place shadow-store protocol is demoted to a disk-constrained fallback; the primary path at real scale is **rebuild-to-new-file + swap**, which also *eliminates* the in-file epoch-flip transaction.
