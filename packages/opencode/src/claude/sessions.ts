...
export type ResumeDecision = {
  ...
  readonly historyTransfer?: { readonly messages: ReadonlyArray<{ role: string; content: string }>; readonly truncated: boolean }
}

// ── Pure helpers (no I/O, fully testable) ──

export function hashSettings(settings: unknown): string {
  const normalized = JSON.stringify(settings ?? {}, Object.keys(settings as any).sort?.() ? Object.keys(settings as any).sort() : undefined) ?? "{}"
...
export function validateBinding(binding: Binding, ctx: ValidationContext): ValidationResult {
  ...
  if (binding.modelFamily !== ctx.modelFamily) {
    return { valid: false, reason: "model_mismatch", message: `model family mismatch: ${binding.modelFamily} vs ${ctx.modelFamily}` }
  }
...
export function decideResume(input: {
  ...
  if (!validated.valid) {
    ...
    if (validated.reason === "transcript_missing" && canTransfer) {
      ...
      return { strategy: "historyTransfer", binding: invalidate(input.binding, validated.reason), reason: validated.message, historyTransfer: bounded }
    }
...
export function boundHistory(messages: ReadonlyArray<{ role: string; content: string }>): { messages: ReadonlyArray<{ role: string; content: string }>; truncated: boolean } {
...
export function makeMemoryStorage(): BindingStorage & { map: Map<string, Binding> } {
  ...
  return {
    ...
    read: (key) => {
      ...
      if (!v) return Effect.fail(new SessionBindingError({ code: "not_found", message: `binding not found: ${keyOf(key)}` }))
...
export const saveBinding = (storage: BindingStorage, binding: Binding) => storage.write(bindingKey(binding.projectID, binding.openCodeSessionID), binding)
...
export const resolveResumeEffect = (input: {
  ...
  Effect.gen(function* () {
    ...
    const decision = decideResume({ binding, ctx: input.ctx, historyMessages: input.historyMessages })
