...
export const BrowserReactInspectTool = Tool.define(
  ...
  Effect.gen(function* () {
    ...
    return {
      description:
        "Inspect the React component behind a target element in the live browser: component name, dev-build source file/line, current props, and readable hook state (useState/useReducer values), plus a bounded ancestor-component breadcrumb. Reads React's Fiber tree directly — no React DevTools extension required, works on any page the browser already has open. Only useful against a page that is actually a React app running an unminified/development build (hasReact: false and an empty component otherwise); pair with browser_profiler_start/stop for render-count profiling instead of one-shot inspection. PRIMARY targeting is a snapshot ref + snapshotVersion; locator/coords are escape hatches.",
