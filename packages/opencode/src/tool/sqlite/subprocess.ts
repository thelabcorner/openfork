...
import { withHeavyProcessSlot } from "@/tool/heavy-process-concurrency"
  ...
  for (const candidate of candidates) {
    const probe = yield* Effect.scoped(
      Effect.gen(function* () {
        ...
        const out = yield* AppProcess.collectStream(handle.stdout, undefined).pipe(
          Effect.map((r) => r.buffer.toString("utf8").trim()),
        )
...
export const runRemoteAction = Effect.fn("SqliteSubprocess.runRemoteAction")(function* (
  ...
  return yield* withHeavyProcessSlot(
    Effect.gen(function* () {
      ...
      yield* Effect.forkScoped(
        Effect.callback<void>((resume) => {
          if (abort.aborted) {
            resume(
              Effect.flatMap(handle.kill({ forceKillAfter: "3 seconds" as const }), () => Effect.void).pipe(
                Effect.orDie,
              ),
            )
            return
