...
import { ToolBadge, ToolBoundedList, ToolEmpty, ToolRow } from "./tool-parts"
...
type SkillListItem = { name: string; description: string; score?: string }
...
function SkillContentCard(props: { skill: ParsedSkill }) {
  ...
  return (
    <div data-component="skill-content-card">
      <div data-slot="skill-content-header">
        ...
        <Show when={props.skill.files.length}>
          <span data-slot="skill-content-file-count">
            {props.skill.files.length} {i18n.t(props.skill.files.length > 1 ? "ui.tool.skill.files.other" : "ui.tool.skill.files.one")}
          </span>
        </Show>
      </div>
      <Show when={props.skill.baseDir}>
        <div data-slot="skill-content-basedir" title={props.skill.baseDir}>
          {props.skill.baseDir}
        </div>
      </Show>
      <Show when={props.skill.files.length}>
      ...
      <div data-slot="skill-content-body">
