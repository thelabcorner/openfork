...
import { formatNumber, formatTokens, formatUSD } from "./usage-format"
        ...
        >
          <defs>
            <linearGradient id="usage-area-fill" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stop-color="var(--v2-text-text-accent)" stop-opacity="0.28" />
              <stop offset="100%" stop-color="var(--v2-text-text-accent)" stop-opacity="0" />
            </linearGradient>
          </defs>
          <path d={areaPath()} fill="url(#usage-area-fill)" />
          <polyline points={pointsAttr()} fill="none" stroke="var(--v2-text-text-accent)" stroke-width="1.5" stroke-linejoin="round" />
          <Show when={hovered()}>
            {(point) => (
              <line
                x1={point().x}
...
export function UsageBarRow(props: {
  ...
  return (
    <TooltipV2 placement="top" value={<UsageTooltipContent title={props.label} rows={rows()} />}>
            ...
            >
              <div class="flex flex-1 flex-col items-center gap-1">
                <div
                  ...
                  style={{
                    height: height(),
                    "background-color": bucket.messages > 0 ? "var(--v2-text-text-accent)" : "var(--v2-background-bg-layer-03)",
                  }}
                />
              </div>
            </TooltipV2>
...
export function UsageHeatmap(props: { days: Summary["days"]; metric: Metric }) {
  ...
  const weeks = () => {
    ...
    for (let i = 0; i < firstDay; i++) week.push({ start: 0, messages: 0, cost: 0, tokens: 0, level: 0 })
  ...
  const fillFor = (level: number) =>
    level === 0
      ...
      : `color-mix(in srgb, var(--v2-text-text-accent) ${15 + level * 21}%, transparent)`
                    ...
                    <TooltipV2
                      ...
                      value={
                        <UsageTooltipContent
                          ...
                          rows={[
                            { label: language.t("usage.table.requests"), value: formatNumber(cell.messages, language.intl()) },
