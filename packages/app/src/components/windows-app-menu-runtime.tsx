...
export function WindowsAppMenuRuntime(props: {
  ...
  const runAction = (action: DesktopMenuAction) => {
    const focused = props.lastFocused()
    if (action.startsWith("edit.") && focused?.isConnected) focused.focus({ preventScroll: true })
  ...
  return (
    <DropdownMenu
      ...
      getAnchorRect={() => props.anchor()?.getBoundingClientRect()}
      gutter={4}
      modal={false}
