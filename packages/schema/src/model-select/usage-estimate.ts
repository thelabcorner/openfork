...
// Structural subset of the fork usage window (the renderer's
// `ForkWindowUsage`). Only the two USD figures participate in the estimate, so
// the shared module never depends on the full renderer-side shape.
export type UsageWindowBudget = {
  spentUSD: number
  limitUSD: number
}
...
export function estimateRequestsRemainingFromCost(window: UsageWindowBudget, costPerRequest: number): number | undefined {
