/**
 * ANSI parser for shell output.
 *
 * Ported from `packages/session-ui/src/components/shell-output.tsx` so a shell
 * call renders the same on both surfaces. Two things matter here and neither is
 * cosmetic:
 *
 *  1. **SGR state is real.** `npm`, `vitest`, `tsc`, ripgrep and `git` all emit
 *     colour, and that colour *is* the information — it is how a test runner
 *     separates a pass from a fail and how `git log --graph` separates branches.
 *     Stripping the escapes (what this file replaced) throws that away and
 *     leaves a wall of undifferentiated grey.
 *
 *  2. **A lone `\r` is not a newline.** Progress output redraws one line by
 *     returning to column zero. Treating it as a line break turns a single
 *     progress line into hundreds of near-identical rows and buries whatever
 *     came after it.
 *
 * The parser walks the string once, tracking style state, and emits merged runs
 * of text that share an effective style — so the DOM cost is proportional to
 * the number of *colour changes*, not the number of bytes.
 */
...
function readExtendedColor(parameters: SgrParameter[], index: number) {
  ...
  if (mode === 5 && current.length >= 3) {
    return { color: color256(current[2] ?? -1), nextIndex: index + 1 }
  }

  if (mode === 2 && current.length >= 5) {
    return { color: colorRgb(current[2] ?? -1, current[3] ?? -1, current[4] ?? -1), nextIndex: index + 1 }
  }
