...
function reply(source, response) {
  if (source === "native") {
    // NativePortV2 handles host->ext framing; here we are ext->host, but native host expects type:response
    try { getNativePort().port?.postMessage?.({ type: "response", response }) } catch {}
    // Also: the NativePortV2.pending resolver already handles native-initiated requests;
    // this path is for extension-initiated handling if host pushed a command.
    // For native, host pushes {type:request} and we reply {type:response}.
    // The actual transport is the Port's postMessage — we use the raw port if available.
  } else if (source === "ws") {
...
async function dispatchOperation(tabIdStr, operation, sessionId) {
  ...
  switch (name) {
    ...
    case "screenshot": {
      // Prefer CDP Page.captureScreenshot (fullPage support). Fallback to tabs.captureVisibleTab.
      try {
        const res = await dm.sendCommand(tabId, "Page.captureScreenshot", { format: input.format ?? "png", captureBeyondViewport: !!input.fullPage })
        return { screenshot: { tabId: String(tabId), url: (await chrome.tabs.get(tabId)).url ?? "", title: (await chrome.tabs.get(tabId)).title ?? "", mime: input.format === "jpeg" ? "image/jpeg" : "image/png", data: res.data, width: 1280, height: 800, viewport: { width: 1280, height: 800, dpr: 1, scrollX: 0, scrollY: 0 }, capturedAt: Date.now() } }
      } catch {
        const dataUrl = await chrome.tabs.captureVisibleTab({ format: input.format ?? "png" })
        const data = dataUrl.split(",")[1] ?? ""
        return { screenshot: { tabId: String(tabId), url: "", title: "", mime: input.format === "jpeg" ? "image/jpeg" : "image/png", data, width: 1280, height: 800, viewport: { width: 1280, height: 800, dpr: 1, scrollX: 0, scrollY: 0 }, capturedAt: Date.now() } }
      }
    }
    case "click": {
      const target = input.target
      const coords = await resolveTargetToCoords(tabId, target)
      // Notify overlay (content.js) to show cursor motion
      try { await chrome.tabs.sendMessage(tabId, { type: "opencode:cursor", x: coords.x, y: coords.y, phase: "move" }) } catch {}
      await new Promise(r => setTimeout(r, 40))
      await dm.sendCommand(tabId, "Input.dispatchMouseEvent", { type: "mousePressed", x: coords.x, y: coords.y, button: input.button ?? "left", clickCount: input.clickCount ?? 1 })
      await dm.sendCommand(tabId, "Input.dispatchMouseEvent", { type: "mouseReleased", x: coords.x, y: coords.y, button: input.button ?? "left", clickCount: input.clickCount ?? 1 })
      try { await chrome.tabs.sendMessage(tabId, { type: "opencode:cursor", x: coords.x, y: coords.y, phase: "click" }) } catch {}
      return { clicked: { target: { kind: "coords", center: coords }, coords, clickCount: input.clickCount ?? 1 } }
    }
    case "type": {
...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  ...
  if (m.type === "opencode:request" && m.request) {
    void handleBrokerRequest(m.request, "runtime").then(resp => {
      // resp is sent via handleBrokerRequest's reply; for runtime we need sendResponse
      // Instead, dispatch directly and respond:
    })
    // Actual runtime request/response path:
    void (async () => {
      const startedAt = Date.now()
      try {
...
// Wire native Port's host->ext request handler:
// The host pushes {type:"request", request: BrokerRequest}. We handle via handleBrokerRequest and reply {type:"response"}.
// Patch getNativePort to intercept messages as requests — NativePortV2 currently treats host as responder,
// but for extension-as-server we need the inverse. We attach a secondary listener on the raw port.
function wireNativeHostRequests() {
  const portWrapper = getNativePort()
  const rawPort = portWrapper.port
  if (!rawPort) return
  // Intercept before NativePortV2's listener by adding another — both will fire; we filter type:request here
  rawPort.onMessage.addListener((raw) => {
    const msg = raw
    if (msg?.type === "request" && msg.request) {
      void (async () => {
        ...
        try {
          if (!isBrokerRequest(req)) throw Object.assign(new Error("Invalid BrokerRequest"), { tag: "BrowserOperationFailed" })
          const result = await dispatchOperation(req.tabId, req.operation, req.sessionId)
          const resp = { ok: true, requestId: req.requestId, result, elapsedMs: Date.now() - startedAt }
          try { rawPort.postMessage({ type: "response", response: resp }) } catch {}
        } catch (err) {
          const resp = { ok: false, requestId: req.requestId ?? "", elapsedMs: Date.now() - startedAt, error: toBrokerErrorBody(err) }
          try { rawPort.postMessage({ type: "response", response: resp }) } catch {}
        }
      })()
    }
...
// Delay wire until port exists
setTimeout(wireNativeHostRequests, 1000)
